...
contract Domains is ERC721URIStorage {
    ...
    // "mapping"でstring型の各keyとaddress型の各データを紐付けにして格納。そのmappingをここでは"domains"として定義。
    mapping(string => address) public domains;

    // stringとstringを紐付けた新しいmapping
    ...
    function price(string calldata name) public pure returns (uint) {
        ...
        // 3文字のドメインの場合
        if (len == 3) {
            return 0.005 * 10**18;  // 5 MATIC = 5 000 000 000 000 000 000
        }
        else if (len == 4) {
    ...
    // register関数はnameとアドレスを紐付ける。
    // calldataは一時的なデータロケーションで不変。
    function register(string calldata name) public payable {
        ...
        // JSON形式のNFTのメタデータを作成。文字列を結合しbase64でエンコード。
        ...
        // newRecordId にNFTをミント
        _safeMint(msg.sender, newRecordId);
        // ドメイン情報をnewRecordIdにセット
    ...
    // nameに対応するaddressを返すゲッター関数を定義。
    function getAddress(string calldata name) public view returns (address) {
        return domains[name];
    }

    function setRecord(string calldata name, string calldata record) public {
        // トランザクションの送信者であることを確認
